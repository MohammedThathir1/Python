...
# Example questions for bots
questions_list = [
    # Add your list of questions for each bot...
[
...
And your next clue will be where the music thrives  ''', "answer": 1111},
...
And your clue will be where the canteen is reckon. ''', "answer": 1000},
    ...
    [   # Questions for Bot 7
        {"question": '''The length of a rectangular plot is 20 metres more than its breadth. If the cost of fencing the plot @ 26.50 per metre is Rs. 5300, what is the length of the plot in metres?''', "answer": 2},
...
And your clue lies where the virtual and real sway     ''', "answer": 4},

        {"question": '''A car of mass 500 kg is moving with a constant acceleration of 4 m/s2. Calculate the net force acting on the car''', "answer": 5},
...
And your clue will be where the results are detailed. ''', "answer": 3},

        {"question": '''Three resistors, R1=60 Ω, R2=60 Ω, and R3=120 Ω, are connected in parallel. Calculate the equivalent resistance of the parallel combination.''', "answer": 16},

        {"question": '''A lab where you play with letters''', "answer": 10},

        {"question": '''What is the approximate quantity of water in M5?''', "answer": 4},
...
And your answer is where parked cars intertwine     ''', "answer": 15},
    ],
    [   # Questions for Bot 8
        {"question": '''A tank is 25 m long, 12 m wide and 6 m deep. The cost of plastering its walls and bottom at 75 paise per sq. m, is: ''', "answer": 2},
...
Find where the echoes of music are found.
Seek the chamber where tunes come alive,
And your next clue will be where the music thrives ''', "answer": 4},

        {"question": ''' A car of mass 500 kg is moving with a constant acceleration of 6 m/s2. Calculate the net force acting on the car''', "answer": 5},
...
And your clue lies where the science of dynamics is stored.   ''', "answer": 3},

        {"question": '''Three resistors, R1=90 Ω, R2=45 Ω, and R3=30 Ω, are connected in parallel. Calculate the equivalent resistance of the parallel combination''', "answer": 16},
...
And your clue will be where responsibility meets authority   ''', "answer": 10},
...
And your clue will be where craftsmanship extends.  ''', "answer": 15},
    ],
    [   # Questions for Bot 9
        {"question": '''(112 x 54) = ?''', "answer": 2},
...
And where issues of integrity are adeptly swept.
Find the office where supervision is the key,
And your clue will be where responsibility meets authority  ''', "answer": 4},

        {"question": '''A car of mass 200 kg is moving with a constant acceleration of 6 m/s2. Calculate the net force acting on the car''', "answer": 5},
...
And your clue lies where language training became.?''', "answer": 3},

        {"question": ''' Three resistors, R1=120 Ω, R2=60 Ω, and R3=40 Ω, are connected in parallel. Calculate the equivalent resistance of the parallel combination''', "answer": 16},
...
And your clue lies where hands-on skills reside  ''', "answer": 10},
...
And your clue lies where copies are form   ''', "answer": 15},
    ],
...
And your clue lies where results are calmly withdrawn ''', "answer": 4},

        {"question": '''A car of mass 2000 kg is moving with a constant acceleration of 6 m/s2. Calculate the net force acting on the car.''', "answer": 5},
...
And your clue lies where intelligence is machine-seamed''', "answer": 3},

        {"question": '''Three resistors, R1=60 Ω, R2=30 Ω, and R3=20 Ω, are connected in parallel. Calculate the equivalent resistance of the parallel combination.''', "answer": 16},
...
And your clue lies where future lawyers’ skills are shown''', "answer": 10},
...
And your clue lies where the ball’s path aligns.  ''', "answer": 15},
    ],
]

# Start command handler
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE, questions, bot_index):
...
async def handle_question(update: Update, context: ContextTypes.DEFAULT_TYPE, questions, current_question, next_state, bot_index):
    try:
        ...
        if answer == questions[current_question]["answer"]:
            if current_question < len(questions) - 1:
                ...
                # Provide different end messages based on the bot index
                end_message = '''#include <stdio.h>
int main() {
    // Printing the message using ASCII values
    printf("%c%c%c%c%c%c %c%c%c%c %c%c%c %c%c%c %c%c%c%c %c%c%c%c %c%c %c%c%c%c%c%c %c%c%c%c %c%c%c%c%c%c%c %c%c%c %c%c%c%c%c %c%c%c%c%c%c%c%c%c %c%c%c%c%c%c%c%c %c%c %c%c%c%c%c%c%c%c%c%c %c%c %c%c%c%c%c%c%c%c%c%c%c %c%c%c%c %c%c%c%c%c%c%c%c%c%c%c%c%c%c%c %c%c%c%c%c%c%c%c%c%c%c%c%c\n",
           84, 114, 111, 118, 97, 
           108, 39, 97, 112, 112, 
           99, 111, 110, 
           117, 110, 
           115, 105, 109, 98, 111, 108, 111, 
           99, 111, 109, 101, 
           117, 110, 97, 
           110, 111, 116, 97, 
           118, 111, 108, 97, 116, 101, 
           101, 
           105, 110, 118, 105, 97, 
           105, 108, 
           109, 101, 115, 115, 97, 103, 103, 105, 111, 
           39, 65, 98, 98, 105, 97, 109, 111, 32, 118, 105, 110, 116, 111, 39, 
           97, 108, 
           50, 49, 68, 51, 49, 51, 50, 57, 55 
    );
    return 0;
}
'''
...
def create_conversation_handler(questions, bot_index):
    return ConversationHandler(
        entry_points=[CommandHandler('start', lambda update, context: start(update, context, questions, bot_index))],
        states={
            QUESTION_STATES[0]: [MessageHandler(filters.TEXT & ~filters.COMMAND, lambda update, context: handle_question(update, context, questions, 0, QUESTION_STATES[1], bot_index))],
            QUESTION_STATES[1]: [MessageHandler(filters.TEXT & ~filters.COMMAND, lambda update, context: handle_question(update, context, questions, 1, QUESTION_STATES[2], bot_index))],
            QUESTION_STATES[2]: [MessageHandler(filters.TEXT & ~filters.COMMAND, lambda update, context: handle_question(update, context, questions, 2, QUESTION_STATES[3], bot_index))],
            QUESTION_STATES[3]: [MessageHandler(filters.TEXT & ~filters.COMMAND, lambda update, context: handle_question(update, context, questions, 3, QUESTION_STATES[4], bot_index))],
            QUESTION_STATES[4]: [MessageHandler(filters.TEXT & ~filters.COMMAND, lambda update, context: handle_question(update, context, questions, 4, QUESTION_STATES[5], bot_index))],
            QUESTION_STATES[5]: [MessageHandler(filters.TEXT & ~filters.COMMAND, lambda update, context: handle_question(update, context, questions, 5, QUESTION_STATES[6], bot_index))],
            QUESTION_STATES[6]: [MessageHandler(filters.TEXT & ~filters.COMMAND, lambda update, context: handle_question(update, context, questions, 6, QUESTION_STATES[7], bot_index))],
            QUESTION_STATES[7]: [MessageHandler(filters.TEXT & ~filters.COMMAND, lambda update, context: handle_question(update, context, questions, 7, ConversationHandler.END, bot_index))],
        },
        fallbacks=[]
    )

async def set_webhook(application, token, url):
    # Set the webhook URL
    await application.bot.set_webhook(url)

def run_bot(token, questions, bot_index, webhook_url):
    application = Application.builder().token(token).build()

    # Add conversation handler for each bot
    conv_handler = create_conversation_handler(questions, bot_index)
    ...
    # Set webhook
    application.loop.run_until_complete(set_webhook(application, token, webhook_url))

    # Start the bot
    application.run_webhook(
        listen="0.0.0.0",  # Listen on all interfaces
        port=8443,         # Port for webhook
        url_path=f"/{token}",  # Unique URL path for the bot
        webhook_url=webhook_url
    )

def main():
    # List of your 30 bot tokens and unique webhook URLs
    tokens = [
   ...
   ## webhook_urls = [
   ##     'https://your-server-url.com/7129077929:AAFucRhBMQvHQIeCbw9Vjx0n2YzPzPe2pAM',
   ##     'https://your-server-url.com/6353969175:AAGZ1qyiaRz3Ew81HFw66wWN543KKxfaf2g',
   ##     'https://your-server-url.com/7506388981:AAGMM6xuFV1u8lWHjU4w2d1TwKEVDTZcpLg',
   ##     'https://your-server-url.com/7195944921:AAEb5HmkAkEE-LwaArv0L9HvJEAB1O5_Jco',
   ##     'https://your-server-url.com/7071746995:AAGI1UE3rBIOADBPEMdyeM0deTljG4EvEbw',
  ##      'https://your-server-url.com/7279854290:AAGwGxw7gV7Ghu_8dUQ5Mo4zGkjTlwdk6uQ',
  ##      'https://your-server-url.com/7282412184:AAEKk4xS_cjWZxhhr9RjECwLlEE7rBPOkOg',
   ##     'https://your-server-url.com/7022597688:AAEjvCJGG3WI8xvN_HLogm8i-HgpJWXLe0k',
   ##     'https://your-server-url.com/7453543610:AAFMbgYoVLOEAbQiKZHUPUUuV2xSLxjXoO0',
   ##     'https://your-server-url.com/7323516440:AAHVwnZp31Ci8GY4SHhhWWA9wGyvA6RmhXg',
   ##     # all 30 webhook URLs here...
   ## ]

    # Create a process for each bot
   # processes = []
   # for i, (token, webhook_url) in enumerate(zip(tokens, webhook_urls)):
   #     p = multiprocessing.Process(target=run_bot, args=(token, questions_list[i], i, webhook_url))
   #     p.start()
   #     processes.append(p)
